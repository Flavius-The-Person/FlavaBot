...
public class ChannelNotificationOnSubscription {
	...
	public ChannelNotificationOnSubscription(EventManager eventManager) {
		...
		String eventString = eventManager.getEventHandler(SubscriptionEvent.class).getSubscriptionPlan();
		//eventManager.getEventHandler(SubscriptionEvent.class).subscribe(event -> onSubscription(event));
	}
